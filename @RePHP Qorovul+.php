...
if(mb_stripos($text,"@Coder_cash") !== false){
$gname = $message->chat->title;
$cusername = $message->chat->username;
$name = $message->from->first_name;
  bot('sendmessage',[
...
if((stripos($mtext,"Erkamoy") !== false) or (stripos($mtext,"erkamoy") !== false) or (stripos($mtext,"erikk") !== false) or (stripos($mtext,"Erik")!==false)){
...
/*sozlama menyusi*/
if($text1 == "/sozlama" or $text == "/sozlama@GroupMixBot"){
...
if($text1 == "/silent@GroupMixBot" &&$fadmin==$admin){
	 if($cty == "group" or $cty == "supergroup"){
...
'reply_to_message_id'=>$re_msgid,
 'parse_mode'=>'markdown',
   'reply_markup'=>json_encode([
	...
	[
	['text'=>"Kanalimiz",'url'=>"https://telegram.me/rephp"]
	]
    ]
    ])
]);
...
if($text1 == "/unsilent@groupmixbot" &&$fadmin==$admin){
 if($cty == "group" or $cty == "supergroup"){
 bot('restrictChatMember',[
   'user_id'=>$rtid,   
   'chat_id'=>$chat_id,
...
😎 @RePHP - Yangiliklar Kanali 💟

🤖 @GroupMixBot - Sinab Ko'ring 😈
...
# Ushbu Kod @GroupMixBot ning eng yanggi versiyasi hisoblanadi #

if($text == "/start" or $text == "/start@GroupMixBot"){
...
[['text'=>"🤖Botlarimiz🤖", 'callback_data'=>"cash"],['text'=>"📢 Reklama Berish 📢", 'callback_data'=>"ads"]],
[['text'=>"💎 Statistika 💎", 'callback_data'=>"static"],['text'=>"☃️ Yangi Yil ☃️", 'callback_data'=>"newyear"]],
[['text'=>'💣 Guruhga Qoshish','url'=>'telegram.me/GroupMixBot?startgroup=new']],
]   
]),
]);
bot('sendMessage',[
...
'disable_web_page_preview'=>true,
  'reply_markup'=>json_encode([   
   'inline_keyboard'=>[ 
[['text'=>'📮 Reklama Berish 📮', 'url' => "t.me/Arslon_777"]],
]   
]),
]);
}
...
🎯Yordam kerak bo'lsa - @Coder_cash
...
'reply_markup'=>json_encode(
['inline_keyboard' => [
[['text'=>'Quron','url'=>'http://t.me/Quronukarimrobot']],
[['text'=>'Dice','url'=>'https://t.me/Ogabek_portfolio_bot']],
[['text'=>'Orqaga','callback_data'=>'test']],
]
]),
]);
...
📊 Narxlar:
 📋Habarnoma: Foydalanuvchilarga - <b>$narxus so'm</b>
 📋Habarnoma: Guruhlarga - <b>$narxgr so'm</b>
 📋Habarnoma: Umumiy - <b>$narxobsh so'm</b>
 🎁PRIME -  <b>10000so'm!</b>
...
'reply_markup'=>json_encode(
['inline_keyboard' => [
[['text'=>'Admin','url'=>'http://t.me/Arslon_777']],
[['text'=>'Orqaga','callback_data'=>'test']],
]
]),
]);
...
[['text'=>"🕰 Dunyo Soatlari 🕰",'callback_data'=>"soat"]],
[['text'=>"🆔 ID Bilan Aniqlash 🔍",'callback_data'=>"id "]],
[['text'=>"🔺 Tarqatma Tayyorlash 🔺",'callback_data'=>"share "]],
[['text'=>"🔰 Haqida Ma'lumot 🔰",'callback_data'=>"@ "]],
[['text'=>'🔙Orqaga','callback_data'=>'test']],
]
]),
]);
...
if($data=="test"){
   ...
   'chat_id'=>$chat_id2,
    'message_id'=>$message_id2,
    'text'=> "😊Assalomu alaykum $ufname $uname ❗️
😍Xush kelibsiz, Men Qorovul bo'laman🖐
💟Meni ko'proq guruhlarga qo'shing va o'sha guruh eng zo'r guruh bo'ladi💥
...
if(mb_stripos($text,"/sms@GroupMixBot") !== false){
$ex = explode(" ",$text);
...
bot('restrictChatMember',[
 ...
 if($text1=="/adminlist" or $text1 == "/adminlist@GroupMixBot"){
...
if($mtext=="/screen" or ($mtext=="/screen@GroupMixBot")){
bot('sendmessage',[
'chat_id'=>$chat_id,
...
'photo'=>"https://api.site-shot.com/?url=$ex[1]",
		...
		if(stripos($mtext,"/soat@GroupMixBot") !== false){
		$soat = date('H:i', strtotime('0 hour'));
  ...
  $a=json_encode(bot('sendmessage',[
   ...
   'parse_mode' => 'markdown'
		...
		if(stripos($mtext,"/sana@GroupMixBot") !== false){
...
if(stripos($mtext,"/id@GroupMixBot") !== false){
...
if(stripos($mtext,"/gid@GroupMixBot") !== false){
...
if($text1 == "/ro" or $text1 == "/roGroupMixBot" or $text1 == "RO" or $text1 == "rO"){
$gett = bot('getChatMember', [
'chat_id' => $chat_id,
'user_id' => $fadmin,
]);
...
if($get =="administrator" or $get == "creator"){
  ...
  bot('sendmessage', [
    ...
    if($text1 == "/Kick"  or  $text1 == "kick"  or $text1 == "/kick@GroupMixBot"){
$gett = bot('getChatMember', [
'chat_id' => $chat_id,
'user_id' => $fadmin,
]);
...
if($text1 =="/banGroupMixBot" or $text1 == "Sur"&&$fadmin==$admin){
...
if($text1 == "/fulladmin@GroupMixBot"){
$gett = bot('getChatMember', [
'chat_id' => $chat_id,
'user_id' => $fadmin,
]);
...
bot('promoteChatMember',[
  ...
  if($text1 == "/unro" or $text1 == "UNRO" or $text1 == "/unro@GroupMixBot"){
 $gett = bot('getChatMember', [
'chat_id' => $chat_id,
'user_id' => $fadmin,
]);
...
if($text1 == "/pin" or $text1 == "/pin@GroupMixBot" or $text1 == "PIn" or $text1 == "PIN" or $text1 == "piN" or $text1 == "pIN" or $text1 == "pIn" or $text1 == "pIN"){
$gett = bot('getChatMember', [
'chat_id' => $chat_id,
'user_id' => $fadmin,
]);
...
if($text1=="/leave@GroupMixBot"&&$fadmin==$admin) {
  bot('sendmessage', [
      'chat_id' => $chat_id,
...
if((stripos($mtext,"ping")!==false) and $fadmin == $admin){
 ...
 if($text1 == "ban" or $text1 == "Ban" or $text1== "/ban@GroupMixBot" or  $text1 == "/ban"){
$gett = bot('getChatMember', [
'chat_id' => $chat_id,
'user_id' => $fadmin,
]);
...
if($get =="administrator" or $get == "creator"){
 ...
 if($text1 == "/unban@GroupMixBot"  or  $text1 == "/unban"){
$gett = bot('getChatMember', [
'chat_id' => $chat_id,
'user_id' => $fadmin,
]);
...
if($text1 == "/warn" or $text1 == "Warn" or $text1 == "/warn@GroupMixBot"){
  $gett = bot('getChatMember', [
'chat_id' => $chat_id,
'user_id' => $fadmin,
]);
...
bot('sendChatAction',['chat_id'=>$chat_id,'action'=>"typing"]);
  ...
  if($text1 == "/unwarn@GroupMixBot" or $text1 == "Unwarn" or $text1 == "/unwarn"){
  $gett = bot('getChatMember', [
'chat_id' => $chat_id,
'user_id' => $fadmin,
]);
...
if($text=="/inline" or $text=="/inline@GroupMixBot"){
  $text = "⚡️Botimiz INLINE rejimida ham xizmat ko'rsata oladi💥
❗️Bu imkoniyat sizga kanallaringizga ham ushbu xizmatlarni taqdim etadi❕
👇Quyidagi tugmalardan foydalaning👇";
  bot('sendmessage',[
...
if(mb_stripos($query,"share")!==false){
	$ex=explode(" ",$query);
	$tarqatma = str_replace("@GroupMixBot share","",$query);
  bot('answerInlineQuery',[
    'inline_query_id'=>$iid,
    'cache_time'=>1,
    'results'=>json_encode([[
...
📑 Manosi:</b> <i>$ismm</i>!",
    ...
    'reply_markup'=>[
            'inline_keyboard'=>[
 [['text'=>"by GroupMixBot",'url'=>"https://t.me/GroupMixBot"]],
        ]],
        ]
        ])
]);
...
if(mb_stripos($text,"/post@GroupMixBot") !== false){ 
$ex = explode("-",$text);
bot('sendMessage',[
'chat_id'=>$cid,
'text'=>"$ex[1]",
...
if(mb_stripos($text,"/search@GroupMixBot") !== false){ 
...
if(mb_stripos($text,"/ism@GroupMixBot") !== false){ 
  $ex=explode(" ",$text);
  $ism = file_get_contents("https://ismlar.com/search/$ex[1]");
  $exp = explode('<p class="text-size-5">',$ism);
  $expl = explode('<div class="col-12 col-md-4 text-md-right">',$exp[1]);
...
if(mb_stripos($text,"/musiqa@GroupMixBot") !== false){
$ex=explode(" ",$text);
...
if(mb_stripos($text,"/ovoz@GroupMixBot") !== false){ 
$ex = explode(" ",$text);
...
$m_id = bot('SendPoll',[
'chat_id'=>$c_id,
'question'=>"@GroupMixBot ga qanday baho berardingiz?",
'options'=> json_encode($Poll)
])->result->message_id;
